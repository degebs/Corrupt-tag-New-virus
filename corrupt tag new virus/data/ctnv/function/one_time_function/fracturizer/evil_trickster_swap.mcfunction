...
execute if score @s evil_player_swap_detect matches 1 run scoreboard players set @s player_swap_cooldown 600
execute if score @s evil_player_swap_detect matches 1 run clear @s music_disc_relic
execute if score @s evil_player_swap_detect matches 1 run kill @e[type=item,nbt={Item:{id:"minecraft:music_disc_relic"}}]
...
execute if score @s evil_player_swap_detect matches 1 run tag @s add swap_self
execute if score @s evil_player_swap_detect matches 1 at @s run tag @a[team=runners,limit=1,sort=furthest] add swap_target
...
execute if score @s evil_player_swap_detect matches 1 as @a[tag=swap_self,tag=swap_target] at @s run tag @a[team=runners,limit=1,sort=furthest] add swap_target

# make sure that the player cannot teleport to themselves
execute if score @s evil_player_swap_detect matches 1 run tag @a[tag=swap_self,tag=swap_target] remove swap_target

# --- AUDIO & VISUAL EFFECTS (pre‑swap) ---
# Play enderman teleport sound at both players
execute if score @s evil_player_swap_detect matches 1 as @a[tag=swap_self] at @s run playsound minecraft:entity.enderman.teleport master @a[distance=0..30] ~ ~ ~ 1 0.8
execute if score @s evil_player_swap_detect matches 1 as @a[tag=swap_target] at @s run playsound minecraft:entity.enderman.teleport master @a[distance=0..30] ~ ~ ~ 1 0.8

# Spawn portal particles at both players' original positions
execute if score @s evil_player_swap_detect matches 1 as @a[tag=swap_self] at @s run particle minecraft:portal ~ ~0.5 ~ 0.5 1 0.5 0.5 150
execute if score @s evil_player_swap_detect matches 1 as @a[tag=swap_target] at @s run particle minecraft:portal ~ ~0.5 ~ 0.5 1 0.5 0.5 150

# --- FANCY ANNOUNCEMENT (colorful JSON) ---
# Build the tellraw with player names and colors
execute if score @s evil_player_swap_detect matches 1 run tellraw @a [{"text":"⚡ ","color":"gold","bold":true},{"text":"EVIL SWAP","color":"light_purple","bold":true},{"text":" ⚡ ","color":"gold","bold":true},{"text":"\n","color":"yellow"},{"selector":"@a[tag=swap_self,limit=1]","color":"dark_purple"},{"text":" swapped places with ","color":"white"},{"selector":"@a[tag=swap_target,limit=1]","color":"yellow"},{"text":" !","color":"white"}]

# Summon a marker at the self player's original position
execute if score @s evil_player_swap_detect matches 1 at @s run summon marker ~ ~ ~ {NoGravity:1b,Invulnerable:1b,Tags:["swap_marker"]}

# Teleport the self player to the target player's position
execute if score @s evil_player_swap_detect matches 1 as @a[tag=swap_self,limit=1] at @s run tp @s @a[tag=swap_target,limit=1]

# Teleport the target player to the marker (original self position)
execute if score @s evil_player_swap_detect matches 1 as @a[tag=swap_target,limit=1] at @s run tp @s @e[tag=swap_marker,limit=1]

# --- EFFECTS AFTER TELEPORT (destination) ---
# Spawn end rod and reverse portal particles at the new positions (using marker locations)
execute if score @s evil_player_swap_detect matches 1 at @e[tag=swap_marker,limit=1] run particle minecraft:end_rod ~ ~0.5 ~ 0.3 0.5 0.3 0.1 60
execute if score @s evil_player_swap_detect matches 1 at @e[tag=swap_marker,limit=1] run particle minecraft:reverse_portal ~ ~0.5 ~ 0.3 0.5 0.3 0.1 60

# Play a satisfying teleport sound for the swapping player (and optionally all)
execute if score @s evil_player_swap_detect matches 1 as @s run playsound minecraft:entity.player.teleport master @a[distance=0..30] ~ ~ ~ 0.6 1.2

# (Optional) Face each other after swap – uncomment if desired
# execute if score @s evil_player_swap_detect matches 1 as @a[tag=swap_self] at @s run tp @s ~ ~ ~ facing entity @a[tag=swap_target,limit=1]
# execute if score @s evil_player_swap_detect matches 1 as @a[tag=swap_target] at @s run tp @s ~ ~ ~ facing entity @a[tag=swap_self,limit=1]

# for your greed you shall be set to 1 heart
execute if score @s evil_player_swap_detect matches 1 run scoreboard players set @a[tag=swap_self] health 1

# Clean up tags and marker
execute if score @s evil_player_swap_detect matches 1 run tag @e[tag=swap_self] remove swap_self
execute if score @s evil_player_swap_detect matches 1 run tag @e[tag=swap_target] remove swap_target
execute if score @s evil_player_swap_detect matches 1 run kill @e[tag=swap_marker]

# Set detect back to 0 (so it doesn't repeat)
execute if score @s evil_player_swap_detect matches 1 run scoreboard players set @s evil_player_swap_detect 0
#==============================================================================================================================================================
# end of swap logic
#===============================================================================================================================================================
execute if score @s evil_player_swap_detect matches 1 run scoreboard players set @s evil_player_swap_detect 0
...
execute as @s at @s if score @s player_swap_cooldown matches 0 unless entity @a[team=runners,distance=0.1..3] run item replace entity @s hotbar.2 with music_disc_relic[item_name=[{"text":"Runner swap!","color":"green","italic":false}]]

# this one is different it only works if 2 runners are close enough to eachother (8 blocks) otherwise it will not work

# detect if 2 runners are within 8 blocks
execute as @a[team=runners,limit=1,sort=furthest] at @s unless entity @p[team=runners,distance=0.1..8] run scoreboard players set bool evil_player_swap_condition 0
execute as @a[team=runners,limit=1,sort=furthest] at @s if entity @p[team=runners,distance=0.1..8] run scoreboard players set bool evil_player_swap_condition 1

# if 1v1 situation or endgame make it always 1
execute if score endgame state matches 1 run scoreboard players set bool evil_player_swap_condition 1
execute if score gamers players_online matches ..2 run scoreboard players set bool evil_player_swap_condition 1



execute as @s if score bool evil_player_swap_condition matches 0 if score @s player_swap_cooldown matches 0 run item replace entity @s hotbar.2 with barrier[item_name=[{"text":"Runners are not grouped up!","color":"red","italic":false}]]
