...
#if the tick score reaches 19 decrease the countdown score by 1
execute if score tick time matches 19 run scoreboard players remove countdown time 1
...
execute if score countdown time matches 29 if score tick time matches 18 run tellraw @a ["choose your ",{"text":"runner ","color":"dark_aqua"},"class and ",{"text":"corrupted ","color":"light_purple"},"class           \nif you dont choose with your inventory, you will be given a random class"]
...
execute if score tick time matches 15 run execute as @e[type=armor_stand,scores={spawning_circle=1}] at @s rotated ~5 ~ run summon armor_stand ^ ^ ^4 {Rotation:[0.0f,0.0f],Tags:["spawn_point"],Invisible:1b}
execute if score tick time matches 15 run scoreboard players add @e[tag=spawn_point] spawn_circle_count 1
