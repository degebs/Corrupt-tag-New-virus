...
#| $$| $$  | $$| $$    | $$_____/| $$        | $$ /$$| $$| $$  | $$| $$  | $$      | $$  \ $$
#| $$| $$  | $$| $$    |  $$$$$$$|  $$$$$$$  |  $$$$/| $$|  $$$$$$/| $$  | $$      |  $$$$$$/
#|__/|__/  |__/|__/     \_______/ \_______/   \___/  |__/ \______/ |__/  |__/       \______/ 
                                                                                            
                                                                                           
# this one will play a lot like hide and reap. exept insdead of dying the runners with 0 health just become corrupted. allowing multiple corrupted                                                                                           
#===============================================================================================================
...
execute if score count runners_alive matches 2.. if score tick time matches 1 run scoreboard players add seconds time 1
...
execute if score gamers players_online matches 2.. if score corruption_tick corruption >= @a[limit=1,scores={ST____corruption_rate=1..}] ST____corruption_rate if score tick time matches 19 if score count runners_alive matches 1.. run scoreboard players add infected corruption_combined 1
...
# all corrupted players will have there corruption be this variable "infected"
execute as @a[team=corrupted] run scoreboard players operation @s corruption = infected corruption_combined
#==================================================================================================
...
execute as @a[team=dead] if score @s health matches 0 if score trigger hide_and_reap_death_trigger matches 0 if score @s hide_and_reap_death_trigger matches 1 unless score runner_win hide_and_reap_death_trigger matches 1 run title @s title {"text":"you got corrupted","color":"dark_purple"}
...
execute as @a[team=dead] if score @s health matches 0 if score trigger hide_and_reap_death_trigger matches 0 if score @s hide_and_reap_death_trigger matches 1 unless score runner_win hide_and_reap_death_trigger matches 1 run tellraw @a {"text":"", "color":"white", "extra":[{"selector":"@s","color":"white"},{"text":" got corrupted after: ","color":"white"},{"score":{"name":"hours","objective":"time"},"color":"white"},{"text":" hours, ","color":"gold"},{"score":{"name":"minutes","objective":"time"},"color":"yellow"},{"text":" minutes, ","color":"white"},{"score":{"name":"seconds","objective":"time"},"color":"white"},{"text":" seconds","color":"green"}]}
...
# this code is a big mangled and a much simpler implementation can be used.
# the moral of the stroy here is to NEVER vibe code critical function
scoreboard players set @a[team=runners] runners_alive 0
scoreboard players set count runners_alive 0
...
# have a variable for the runners alive
scoreboard players operation count runners_alive = @a[team=runners] runners_alive
...
execute if score infected corruption_combined matches 70.. run function ctnv:endgame
...
execute as @a[team=corrupted] unless score infected corruption_combined matches ..99 run function ctnv:death
...
# since there are multiple players this step may not be nessasarry
...
#=================================================================================================
# victory condition
# we must detect if the "runners_alive" variable is equal to 0
# then trigger it
execute if score count runners_alive matches 0 run function ctnv:alternate_gamemode_functions/infection_corrupted_victory


#=================================================================================================
# there is still a glitch where multiple corrupted can exist
# here this in intentional so the fix will not be applied here
